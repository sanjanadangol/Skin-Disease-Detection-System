...
def home(request):
    ...
    if request.method == "POST" and request.FILES['affectedphoto']: 
        if request.user.is_authenticated:
            image = request.FILES['affectedphoto']
            # f = Fernet(key)
            # encrypted = f.encrypt(image.encode())
            # print(encrypted)
            addProfile = UserUpload(user= request.user, image= image)
            addProfile.save()
            imagePath = addProfile.image.url
            print("jansjdn" + imagePath)
           
            # encrypt(imagePath.strip('/'),key)
            # decrypted = decrypt_and_decode(imagePath.strip('/'), key, 'utf-8')
            # fullImagePath = 'E:\7th Semester\Final Year Project\New folder\Skin-Disease-Detection-System-Using-CNN-Model-main' + imagePath
            fullImagePath = 'E:\\7th Semester\\Final Year Project\\New folder\\Skin-Disease-Detection-System-Using-CNN-Model-main' + imagePath
            ...
            trainedModel = tf.keras.models.load_model('E:/7th Semester/Final Year Project/New folder/Skin-Disease-Detection-System-Using-CNN-Model-main/trained.h2')
